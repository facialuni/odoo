...
class ResConfigSettings(models.TransientModel, ResConfigModuleInstallationMixin):
    """ Base configuration wizard for application settings.  It provides support for setting
        ...
        *   For a char/boolean field with no specific prefix BUT an attribute 'config_parameter',
    ...
    def _get_classified_fields(self):
        """ return a dictionary with the fields classified by category::
                ...
                {   'default': [('default_foo', 'model', 'foo'), ...],
                    ...
                    'module':  [('config_plop', config_parameter), ...],
                    'other':   ['other_field', ...],
                }
        """
        IrModule = self.env['ir.module.module']
        ...
        for name, field in pycompat.items(self._fields):
            if name.startswith('default_') and hasattr(field, 'default_model'):
                ...
                modules.append((name, module))
            elif hasattr(field, 'config_parameter') and field.type in ('boolean', 'char', 'selection', 'many2one'):
                # TODO support other field types? no real use cases for now
    ...
    def default_get(self, fields):
        ...
        for name, icp in classified['config']:
            value = SuIrConfigParameter.get_param(icp, self._fields[name].default or False)
            if self._fields[name].type == 'many2one' and value is not False:
                try:
                    value = int(value)
                except ValueError:
                    pass
