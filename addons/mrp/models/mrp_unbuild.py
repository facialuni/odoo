...
class MrpUnbuild(models.Model):
    ...
    def action_unbuild(self):
        ...
        try:
            ...
            consume_move.unlink()
            produce_moves.unlink()
            raise UserError(_('You do not have enough of this product to unbuild'))
        ...
        for produce_move in produce_moves:
            if produce_move.has_tracking != 'none':
                if self.mo_id:
                    ...
                    for move_lines in original_move.move_lines_ids:
        ...
        produced_move_lines_ids = produce_moves.mapped('move_line_ids').filtered(lambda ml: ml.qty_done > 0)
        consume_move.move_line_ids.sudo().write({'produce_line_ids': [(6, 0, produced_move_lines_ids.ids)]})
