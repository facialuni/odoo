...
class TemplatePreview(models.TransientModel):
    ...
    language = fields.Many2one('res.lang', string='Choose your language', required=True)
    ...
    def onchange_lang(self):
        ...
        if self.res_id and self._context.get('template_id'):
            ...
            # template.lang = self.language.code
            # self.name = template.name
            mail_values = template.with_context(lang=self.language.code or self.lang).generate_email(self.res_id)
            # print mail_values
