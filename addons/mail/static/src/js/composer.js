...
var BasicComposer = Widget.extend(chat_mixin, {
        ...
        this.$('.o_composer_button_emoji').popover({
            ...
            content: function() {
                if (!self.$emojis) { // lazy rendering
                    ...
                    self.$emojis.filter('.o_mail_emoji').on('click', self, self._onClickEmojiImg.bind(self));
                }
                return self.$emojis;
            },
            html: true,
    ...
    */
    ...
    /**
     * On click of emoji image in chatter.
     * @private
     * @param {KeyboardEvent} event
     */
    _onClickEmojiImg: function (event) {
        var textInput = this.$input.html();
        var pos = this.mention_manager._getSelectionPositions();
        var textLeft = textInput.substring(0, pos);
        var textRight = " " + textInput.substring(pos, textInput.length);
        textLeft += "<span> " + $(event.currentTarget).data('emoji') + " </span>";
        var img = event.currentTarget.cloneNode(true).children[0];
        img.id = nodeID;
        this.$input.html("");
        this.$input.append(textLeft).append(img).append(textRight);
        this.mention_manager._setCursorPostition(nodeID);
        nodeID++;
    },
