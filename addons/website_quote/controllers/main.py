...
from odoo.addons.sale_payment.controllers.main import SaleQuotation
...
class WebsiteSaleQuotation(SaleQuotation):
    ...
    def quote_view(self, order_id, pdf=None, token=None, message=False, **post):
        ...
        if order_sudo and request.session.get('view_quote') != now and request.env.user.share:
        ...
        return super(WebsiteSaleQuotation, self).quote_view(order_id=order_id, pdf=pdf, token=token, **post)
    ...
    def quote_accept(self, order_id, token=None, signer=None, sign=None, **post):
        ...
        if token != Order.access_token or Order.require_payment:
            return request.render('website.404')
        return super(WebsiteSaleQuotation, self).accept(order_id=order_id, token=token, signer=signer, sign=sign, **post)
