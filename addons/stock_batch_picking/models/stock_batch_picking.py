...
class StockBatchPicking(models.Model):
    ...
    picking_ids = fields.One2many(
        'stock.picking', 'batch_picking_id', string='Pickings',
        help='List of picking associated to this batch picking')
    ...
    def done(self):
        ...
        for picking in pickings:
            picking.message_post(
                body="<b>%s:</b> %s <a href=#id=%s&view_type=form&model=stock.batch.picking>%s</a>" % (
                    _("Transferred by"),
                    _("Batch Picking"),
                    picking.batch_picking_id.id,
                    picking.batch_picking_id.name))
        if pickings:
    ...
    def _track_subtype(self, init_values):
        if 'state' in init_values:
            return 'stock_batch_picking.mt_batch_state'
        return super(StockBatchPicking, self)._track_subtype(init_values)
...
class StockPicking(models.Model):
    ...
    batch_picking_id = fields.Many2one(
        'stock.batch.picking', string='Batch Picking',
        states={'done': [('readonly', True)], 'cancel': [('readonly', True)]},
        help='Batch Picking associated to this picking')
