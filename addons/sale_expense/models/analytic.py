...
class AccountAnalyticLine(models.Model):
    ...
    def _sale_postprocess(self, additional_so_lines=None, unlinked_so_lines=None):
        """ Set the SO line before recomputed delivered quantity on SO line.
            Only applied for analytic line from expense posting, having no SO line already set.
        """
        self.filtered(lambda aal: not aal.so_line and aal.product_id and aal.product_id.expense_policy != 'no')._expense_set_sale_order_line()
        return super(AccountAnalyticLine, self)._sale_postprocess(additional_so_lines=additional_so_lines, unlinked_so_lines=unlinked_so_lines)
    ...
    def _expense_set_sale_order_line(self):
        """ Automatically set the SO line on the analytic line, for the expense flow. It retrives
            an existing line, or create a new one (upselling expenses).
        """
        for analytic_line in self.filtered(lambda aal: not aal.so_line):
