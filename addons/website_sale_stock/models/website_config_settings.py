...
class WebsiteConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        IrDefault = self.env['ir.default'].sudo()
        res.update(inventory_availability=IrDefault._get('product.template', 'inventory_availability') or 'never',
                   available_threshold=IrDefault._get('product.template', 'available_threshold') or 5.0)
        return res
