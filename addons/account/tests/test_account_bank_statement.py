...
class TestAccountBankStatement(common.TransactionCase):
    ...
    def test_account_bank_statement_def(self):
        ...
        journal = self.env['account.bank.statement'].with_context({'lang': u'en_US', 'tz': False, 'active_model': 'ir.ui.menu', 'journal_type': 'bank', 'journal_id': 10, 'date': time.strftime("%Y/%m/%d")})._default_journal()
        ...
        # Check account bank statement created
        self.assertTrue(account_statement, 'Account Bank Statement not created')
        # Check account bank statement Opening and Closing balance is 0
        self.assertEquals(account_statement.balance_start, 0.0)
        self.assertEquals(account_statement.balance_end_real, 0.0)

        # Set value for account bank statement line
        vals = {
            'amount': 1000,
            'date': time.strftime('%Y-%m-%d'),
        ...
        account_bank_statement_line = self.env['account.bank.statement.line'].create(vals)
        # Check Account bank statement is created
        self.assertTrue(account_bank_statement_line, 'Account Bank Statement Line not created')
        ...
        self.assertTrue(account, 'Account not created')
        ...
        # Check Bank statement closing balance
        self.assertEquals(account_statement.balance_end_real, 1000)
