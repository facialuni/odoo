...
class Reconciliation(models.AbstractModel):
    ...
    def reconciliation_widget_preprocess(self, statement_ids):
        ...
        sql_query = """SELECT account_bank_statement_line.id
                        FROM account_bank_statement_line
                        WHERE
                            account_bank_statement_line.account_id IS NULL
                            AND not exists (
                                SELECT 1
                                FROM account_move_line
                                WHERE account_move_line.statement_line_id = account_bank_statement_line.id
                            )
                            AND account_bank_statement_line.company_id = %s
                """
        params = (user.company_id.id,)
        if statement_ids:
            sql_query += ' AND account_bank_statement_line.statement_id IN %s'
            params += (tuple(statement_ids),)
        sql_query += ' ORDER BY account_bank_statement_line.id'
        cr.execute(sql_query, params)

        st_lines_left = Statement_line.browse([line.get('id') for line in cr.dictfetchall()])
        ...
        if st_lines_left and stl_to_assign_partner and refs:
            sql_query = """SELECT aml.partner_id, aml.ref, account_bank_statement_line.id
                            FROM account_move_line aml
                                JOIN account_account ON account_account.id = aml.account_id
                                JOIN account_bank_statement_line ON aml.ref = account_bank_statement_line.name
                            WHERE (aml.company_id = %s 
                                AND aml.partner_id IS NOT NULL) 
                                AND (
                                    (aml.statement_id IS NULL AND aml.account_id IN %s) 
                                    OR 
                                    (account_account.internal_type IN ('payable', 'receivable') AND aml.reconciled = false)
                                    )
                                AND aml.ref IN %s
                                """
            params = (user.company_id.id, (st_lines_left[0].journal_id.default_credit_account_id.id, st_lines_left[0].journal_id.default_debit_account_id.id), tuple(refs))
            if statement_ids:
            ...
            for line in results:
                Statement_line.browse(line.get('id')).write({'partner_id': line.get('partner_id')})

        return {
            'st_lines_ids': st_lines_left.ids,
            'notifications': [],
            'statement_name': len(statement_ids) == 1 and Statement.browse(statement_ids[0]).name or False,
    ...
    @api.model
    def get_move_lines_for_reconciliation(self, statement_line_id, partner_id=None, excluded_ids=None, str=False, offset=0, limit=None, additional_domain=None, overlook_partner=False):
