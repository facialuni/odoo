...
class AccountTax(models.Model):
    ...
    def unlink(self):
        ...
        taxes = self.browse(IrDefault._get('product.template', 'taxes_id', company_id=company_id) or [])
        if self & taxes:
            IrDefault.sudo().set('product.template', 'taxes_id', (taxes - self).ids, company_id=company_id)
        taxes = self.browse(IrDefault._get('product.template', 'supplier_taxes_id', company_id=company_id) or [])
        if self & taxes:
