...
QUnit.module('account', {
    ...
    QUnit.test('Reconciliation partial', function (assert) {
        ...
        assert.equal( widget.$('button.o_validate, button.o_reconcile').not(':hidden').length, 0, "should not display the reconcile button");
        ...
        assert.ok( widget.$('button.o_reconcile:not(:hidden)').length, "should display the reconcile button");
        assert.strictEqual( widget.$el.data('mode'), "inactive", "should be inactive mode");
        widget.$('button.o_validate, button.o_reconcile, button.o_no_valid').not(':hidden').trigger('click');
