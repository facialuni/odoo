...
class pos_payment_report(report_sxw.rml_parse):
    ...
    def _pos_payment(self, obj):
        self.total = 0
        data={}
        self.cr.execute("select id from pos_order where id = %s", [obj.id])
        if self.cr.fetchone():
            self.cr.execute ("select pt.name,pp.default_code as code,pol.qty,pu.name as uom,pol.discount,pol.price_unit, " \
                                 ...
                                 "and po.state IN ('paid','invoiced') and to_char(date_trunc('day',po.date_order),'YYYY-MM-DD')::date = current_date and po.id=%s",
                             [obj.id])
