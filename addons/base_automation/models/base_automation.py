...
class BaseAutomation(models.Model):
    ...
    action_server_id = fields.Many2one('ir.actions.server', string='Server Action', required=True, ondelete='cascade')

    trigger = fields.Selection([
        ('on_create', 'Creation'),
        ('on_write', 'Update'),
        ('on_create_or_write', 'Creation & Update'),
        ('on_unlink', 'Deletion'),
        ('on_change', 'Form Modification'),
    ...
    trg_date_calendar_id = fields.Many2one(
        ...
        help="""When calculating a day-based timed condition,
        it is possible to use a calendar to compute the date based on working days.""")

    active = fields.Boolean(string='Active', default=lambda self: True)
    filter_pre = fields.Char(string='Pre Condition', default="[]", required=True)
    filter_post = fields.Char(string='Condition', default="[]", required=True)
    last_run = fields.Datetime(readonly=True, copy=False)

    on_change_fields = fields.Char(string="Fields", help="Comma-separated list of field names that triggers the onchange.")
    ...
    def onchange_trigger(self):
        if self.trigger not in ['on_time']:
    ...
    @api.multi
    def unlink(self):
        result = super(BaseAutomation, self).unlink()
        self._update_cron()
        self._update_registry()
        return result
    ...
    def _update_cron(self):
        ...
        """
        cron = self.env.ref('base_automation.ir_cron_crm_action', raise_if_not_found=False)
    ...
    def _get_actions(self, records, triggers):
        """ Return the actions of the given trigger for records' model. The
    ...
    def _filter_pre(self, records):
        ...
        if len(self.filter_pre) > 4 and records:
            domain = [('id', 'in', records.ids)] + safe_eval(self.filter_pre, self._get_eval_context())
            return records.search(domain).with_env(records.env)
        return records

    def _filter_post(self, records):
        """ Filter the records that satisfy the postcondition of action ``self``. """
        if len(self.filter_post) > 4 and records:
            domain = [('id', 'in', records.ids)] + safe_eval(self.filter_post, self._get_eval_context())
    ...
    def _process(self, records):
        ...
        if 'date_action_last' in records._fields:
            records.write({
                'date_action_last': fields.Datetime.now()
            })

        # execute server actions
        for record in records:
            ctx = {'active_model': record._name, 'active_ids': [record.id], 'active_id': record.id}
    ...
    def _register_hook(self):
        ...
        def make_onchange(action_rule_id):
            ...
            def base_automation_onchange(self):
                action = self.env['base.automation'].browse(action_rule_id).mapped('action_server_id')
                result = {}
                for server_action in action.with_context(active_model=self._name, onchange_self=self):
    ...
    @api.multi
    def create_action_for_menu(self):
        return self.mapped('action_server_id').create_action()

    @api.multi
    def unlink_action_for_menu(self):
        return self.mapped('action_server_id').unlink_action()
    ...
    def _check(self, automatic=False, use_new_cursor=False):
        ...
        for action in self.with_context(active_test=True).search([('trigger', '=', 'on_time')]):
            ...
            if action.filter_post:
                domain = safe_eval(action.filter_post, eval_context)
