...
QUnit.module('Views', {
    ...
    QUnit.test('onchange send only valid x2many lines', function (assert) {
        assert.expect(1);
        this.data.partner.fields.display_name.required = true;
        this.data.partner.onchanges.foo = function (obj) {
            obj.foo = obj.foo + " alligator";
        };
            ...
            mockRPC: function (route, args) {
                if (args.method === "onchange") {
                    assert.deepEqual(args.args[1].p,
                        [[0, false, {"display_name": "valid line", "qux": 12.4}]],
                        "should send the values without invalid lines");
                }
                return this._super(route, args);
            },
        });
        ...
        form.$('.o_field_x2many_list_row_add a').click();
        form.$('.o_field_one2many input:last').focus();
        form.$('.o_field_one2many input:last').val('33.4').trigger('input');

        form.$('input[name="foo"]:first').click();
